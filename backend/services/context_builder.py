"""T-M28-01: Context Builder skeleton (REFACTOR) — gap closure G1-G4.
...
- G1 (AC-1): Obsidian markdown **write** unified API
  ...
  - ADR-012 Memory Tool (services.anthropic_memory_tool) との関係:
    Memory Tool は Claude が tool として動的に呼ぶ working memory 経路.
    本 API は人間 / scripts が直接呼ぶ静的経路. 両者は filesystem を共有.
...
def read_obsidian_note(slug: str, *, vault_dir: Optional[Path] = None) -> Optional[str]:
    ...
    AC-1 unified API の read 側. Memory Tool 経由でも同じ Vault を参照する.
    """
    _validate_slug(slug)
...
def write_obsidian_note(
    ...
    *,
    ...
    AC-1 unified API の write 側 (G1 closure). path traversal 防止 +
    1 MiB cap.
    """
    _validate_slug(slug)
