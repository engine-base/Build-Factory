---
name: integration
description: 複数ブランチ・複数タスクの実装完了後、本体コードベースへ統合するプロセスを設計するスキル。マージ戦略・コンフリクト対応・統合後の動作確認・ロールバック手順を設計する。**v3 採用 (汎用化版)**: **N 並列セッション × N CI gate auto-merge 前提 (project-defined capacity)**。各 task の PR は CI gate 全 pass で機械的に auto-merge されるため、integration スキルの主軸は **(1) Wave 単位の完了集計** (auto-merged / retried / escalated / rolled-back の 4 カテゴリ + drift 検出件数) **(2) Phase gate 開放判定** (Foundation completion → Backend completion → UI completion → Polish completion を mechanical / observable な tool で判定) **(3) drift fix queue 流し込み** (lint 違反検出 → drift task 自動生成 → 次 Wave の drift fix group に追加) **(4) conflict 原因追跡** (file mutex 漏れ → boundary 設計修正)。**distributed-dev の管理 JSON 群 (branch-package.json N 件) と schedule-design の wave-schedule.json と test-verification の gate-config.yml を pull** し、work-package boundary (file mutex) で conflict が事前防止されている前提で運用。conflict 発生は **v3 仕様違反** として原因記録し、boundary 設計を修正。rollback は **task 単位 PR revert + drift fix queue Wave 再起動** (ブランチ単位 reset --hard は禁止)。ブランチをマージしたい・統合を進めたい・コンフリクトが怖い・統合後の品質確認をしたい・リリース前の最終チェックをしたい・並列 Wave 完了を集計したい・Phase gate 開放判定をしたい・drift を fix queue に流したい・wave-integration-report.md を出したい・phase-gate-decision.json を作りたい場面で必ず使うこと。4STEP の対話型プロセスで進み、出力は統合計画書 (Markdown) + 統合管理 JSON + **wave-integration-report.md** (Wave 完了集計レポート) + **phase-gate-decision.json** (Phase ゲート判定) の **4 形式**。
tab: 品質・運用
builtin: true
---
---
