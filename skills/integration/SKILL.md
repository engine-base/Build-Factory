---
name: integration
description: 複数ブランチ・複数タスクの実装完了後、本体コードベースへ統合するプロセスを設計するスキル。マージ戦略・コンフリクト対応・統合後の動作確認・ロールバック手順を設計する。**v3 採用 (2026-05-15〜)**: **30-50 並列 Claude Code セッション × 8 CI gate auto-merge 前提**。各 task の PR は 8 gate 全 pass で機械的に auto-merge されるため、integration スキルの主軸は **(1) Wave 単位の完了集計** (auto-merge 件数 / 連続失敗件数 / drift 検出件数 / rollback 件数) **(2) Phase gate 開放判定** (Phase 0 → 1 / Phase 1 → 1.5 / Phase 1.5 → 2 / Phase 2 release を mechanical / observable な tool で判定) **(3) drift fix Group D Wave への流し込み** (lint #17-19 違反検出 → generate-drift-tickets.py で次 Wave Group D に追加) **(4) conflict 原因追跡** (file mutex 漏れ → boundary 設計修正)。**distributed-dev の管理 JSON 群 (branch-package.json N 件) と schedule-design の wave-schedule.json と test-verification の gate-config.yml を pull** し、work-package boundary (file mutex) で conflict が事前防止されている前提で運用。conflict 発生は **v3 仕様違反** として原因記録し、boundary 設計を修正。rollback は **task 単位 PR revert + Group D Wave 再起動** (ブランチ単位 reset --hard は禁止)。ブランチをマージしたい・統合を進めたい・コンフリクトが怖い・統合後の品質確認をしたい・リリース前の最終チェックをしたい・30-50 並列の Wave 完了を集計したい・Phase gate 開放判定をしたい・drift を Group D に流したい・wave-integration-report.md を出したい・phase-gate-decision.json を作りたい場面で必ず使うこと。4STEP の対話型プロセスで進み、出力は統合計画書 (Markdown) + 統合管理 JSON + **wave-integration-report.md** (Wave 完了集計レポート) + **phase-gate-decision.json** (Phase ゲート判定) の **4 形式**。
tab: 品質・運用
builtin: true
---
---
...
1. **Wave 単位で集計** — 個別ブランチではなく Wave 単位 (W0/W1/.../W7) で auto-merge 件数 / retried / escalated / rolled-back / drift 件数を集計
2. **8 CI gate auto-merge 前提** — 各 task は gate 全 pass で機械的に merge 済。integration スキルは Wave 完了時の集計 + Phase gate 判定 + drift fix の流し込みが主軸
3. **work-package boundary (file mutex) で conflict 事前防止** — 発生は v3 仕様違反 (check-wave-mutex.py 漏れ) として原因記録し、tickets.json の work_package_boundary を修正
4. **Phase gate 判定は mechanical / observable** — Phase 0 → 1 (8 gate + lint #1-19 + AC validator) / Phase 1 → 1.5 (dogfood 完走) / Phase 1.5 → 2 (lint #17-19 0 + REFACTOR 全 done + drift 0) / Phase 2 release (multi-tenant + billing + SLA)
5. **drift fix は Group D Wave に流す** — lint #17-19 違反 → generate-drift-tickets.py → 次 Wave Group D 20% に追加
6. **rollback は task 単位 PR revert** — ブランチ単位 reset --hard は禁止 (30-50 並列で他 task に影響)
...
**このSTEPでやること：**
**Wave 単位** で何件の task が・どの状態 (auto-merged / retried / escalated / rolled-back) で・どんな drift を残しているかを集計する。個別ブランチ単位ではなく **Wave 単位** に集約するのが v3。
...
### 上流出力
- distributed-dev: .claude/branches/*.json (各 task の branch-package.json N 件)
- schedule-design: docs/schedule-design/<date>_v<N>/wave-schedule.json
- test-verification: docs/test-verification/<date>_v<N>/gate-config.yml
...
### auto-merge 集計 (v3 / 4 カテゴリ)
| 状態 | 件数 | 詳細 |
|------|-----|------|
| ✅ auto-merged (8 gate green) | 27 | task ID list |
| ⚠️ retried (1〜2 失敗で recovery) | 2 | task ID + 失敗 gate |
| 🔴 escalated (連続 3 失敗 → human) | 1 | task ID + 失敗 gate + 原因 |
| 🟡 rolled back (post-merge 問題) | 0 | task ID + revert PR |

### conflict 検出 (v3: 仕様違反として原因追跡)
| 発生 task | conflict file | 原因 | 修正方針 |
|---|---|---|---|
| (発生時のみ) | (file mutex 漏れ) | tickets.json 修正 + revert + Group D 再投入 |

### drift 検出 (Group D 行き)
| lint | 件数 | 生成 drift task |
|------|------|----------------|
| #17 mock-impl-diff | 3 | T-DRIFT-W<N>-01〜03 |
| #18 screens-API | 1 | T-DRIFT-W<N>-04 |
...
**このSTEPでやること：**
**v3 では各 task が 8 CI gate auto-merge で機械的に merge されるため**、マージ戦略の設計は不要。代わりに以下を設計:
- **conflict 発生時の原因追跡フロー** (file mutex 漏れ → boundary 設計修正 → revert → drift task 再投入)
- **drift fix の Group D 流し込み** (lint #17-19 違反 → generate-drift-tickets.py → 次 Wave Group D)
...
- merge commit / rebase は使わない (30-50 並列で履歴が破綻するため)
...
1. **conflict 発生 = v3 仕様違反**: check-wave-mutex.py の漏れを意味する
2. **原因調査**:
   - 同 Wave 内に同一 file への editable task が複数 含まれていないか
   - shared_no_concurrent_edit の宣言漏れがないか
3. **boundary 設計修正**: tickets.json の work_package_boundary を更新
4. **revert**: 該当 task の PR を revert (`gh pr create --base main --head revert/<task_id>`)
5. **drift task 再投入**: generate-drift-tickets.py で `T-<task_id>-REDO` を生成 → 次 Wave Group D に追加

### drift fix Group D 流し込みフロー (v3)
1. Wave 完了時に lint #17-19 違反検出 (wave-integration-report.py)
2. 違反箇所ごとに drift task を自動生成 (T-DRIFT-W<N>-<seq>)
3. tickets.json に追加 + 次 Wave (W<N+1>) の Group D 候補リストへ
4. schedule-design.wave-schedule.json を更新 (Group D 20% 割当)
...
gh pr create --base main --head revert/<task_id> \\
  ...
  --body "Reverts #<PR>. See docs/wave-integration/W<N>.md"
gh pr merge revert/<task_id> --auto --squash

# drift task を次 Wave Group D に追加
python3 scripts/generate-drift-tickets.py --task <task_id>-REDO --target-wave W<N+1>
\`\`\`

### 禁止操作 (v3)
- ❌ `git reset --hard` (30-50 並列で他 task に影響)
...
## STEP 3: Phase gate 開放判定 (v3 / mechanical / observable)

**このSTEPでやること：**
Wave 完了の集計を踏まえて、**Phase gate の開放判定** を mechanical / observable な条件で行う。各 Phase 移行は次の通り:

| Phase 移行 | 判定基準 | tool |
|---|---|---|
| **Phase 0 → Phase 1** | 8 CI gate 全 green + lint #1-19 全 0 件 + AC validator pass | scripts/check-phase-gate.py --phase 0 |
| **Phase 1 → Phase 1.5** | dogfood 完走 (Build-Factory 自身を Build-Factory で開発完走) + 187 task 全 merge | scripts/check-dogfood-completion.py |
| **Phase 1.5 → Phase 2** | lint #17-19 全 0 件 + REFACTOR タスク全 done + drift 累積 0 件 | scripts/check-phase-gate.py --phase 1.5 |
...
|------|------|-----------------------|
| 8 CI gate 全 green | green | last 10 commits 全 pass |
| lint #1-19 0 件 | 0 violations | lint-mock.sh exit 0 |
| AC validator pass | pass | validate-tickets.py exit 0 |
...
- next_wave: W<N+1>
- approver: automated (mechanical gate) / human (Phase 2 release のみ)

### 発見されたバグの扱い (Phase 2 release 時のみ)
- 致命的 (P0): リリースブロック → 即修正
- 重要 (P1): リリース判断を要確認
...
```json
{
  ...
  "integration": {
    ...
    "wave_id": "W<N>",
    "phase_id": "P<X>",
    "post_merge_checks": ["8 CI gate", "wave-integration-report.py"],
    "rollback_plan": "task 単位 PR revert + Group D Wave 再起動"
  },
  "status": "ready",
  "next_skill": "delivery"
}
...
## 連続失敗の原因分析
- T-005-03: gate #3 RLS coverage 3 連続失敗
  - 原因: entities.json rls_policies 漏れ
