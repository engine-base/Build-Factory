...
description: スケジュール設計スキル。tickets.json (task-decomposition) + DAG.md / phase-mapping.md (feature-decomposition) をもとに「いつ・何を・誰が・どの条件で納品するか」を設計する。v3 採用 (2026-05-15〜): Foundation phase = Wave 0 を必ず最先行し (project-defined CI gate + lint rule 整備完了まで他 task block)、Foundation / Backend / UI / Polish の段階構成。Sprint = Wave 単位 (1 Wave = 数時間 × project-defined parallel capacity の並列セッション) でガント表を引き、各 task は N CI gate auto-merge で完了判定。連続 N 失敗で human エスカ。Group A (Foundation) / B (Vertical Slice) / C (Integration test) / D (Drift fix) を Wave 内で並列実行。「スケジュールを引きたい」「いつ終わるか決めたい」「フェーズごとの納品物を決めたい」「マイルストーンを設定したい」「リソース配置を決めたい」「請求タイミングを決めたい」「工数見積もりを出したい」「Wave 構成を作りたい」「Foundation phase から段階的に組みたい」「並列 Claude Code セッション前提で組みたい」「CI gate auto-merge 前提で組みたい」「wave-schedule.json を出したい」と言われたら必ず起動する。5STEP の対話型プロセスで進み、出力はスケジュール表 (Markdown) + マイルストーン JSON + wave-schedule.json + 判断ログ JSON の 4 形式。詳細仕様は本文参照。
tab: 実装・分解
builtin: true
---
---
