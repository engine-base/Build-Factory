...
description: アーキテクチャ設計スキル。要件定義・機能一覧・functional-breakdown 出力 (画面/機能/ロール/エンティティ草案) をもとにシステム全体のアーキテクチャ（モノリス/マイクロサービス）・DB 設計方針・**RLS strategy (per-entity policy / tenant isolation / table naming)**・インフラ構成・ライブラリ/OSS 選定・DB ツール選定・開発環境ツール選定を決定する。**v3 採用 (2026-05-15〜)**: アーキテクチャの一部として **Phase 0 Foundation gate** (lint #17-19 / 3-tier AC validator / pyright/coverage gate / mock-impl-diff = 8 merge gate) を定義し、`v3-gate.yml` (GitHub Actions テンプレ) と `phase_0_gates.json` を生成する。30-50 並列 Claude Code セッションを支える monorepo 構造・worktree 戦略・branch 命名規約を STEP 2 で必須検証。ADR-013 (AUTH) / ADR-014 (命名) 等の Phase 0 ADR 起票プロトコルを出力。「アーキテクチャを決めたい」「技術スタックを選定したい」「DB 設計をしたい」「インフラ構成を決めたい」「モノリスかマイクロか判断したい」「スケーラビリティを考慮した設計にしたい」「認証ライブラリを決めたい」「ORM を選びたい」「ホスティング先を決めたい」「CI/CD ツールを選びたい」「全文検索エンジン要否を判断したい」「パッケージマネージャを決めたい」「RLS policy 戦略を決めたい」「CI gate を設計したい」「Phase 0 Foundation を組みたい」「30-50 並列開発対応にしたい」「ADR を起票したい」「monorepo 構造を決めたい」と言われたら、明示されていなくても必ず使う。STEP 1〜5 + STEP 4.5 (選定モジュール = ライブラリ/OSS / インフラ / DB系 / 開発環境/CI ツール) の対話型プロセスで進み、各選定は 2〜3 候補比較表 → ユーザー選択 → AI レビューの形を取る。出力はアーキテクチャ仕様書 (Markdown + HTML) + 設計 JSON + 判断ログ + selected-stack.json + **phase_0_gates.json** + **adrs-to-create.json** + **v3-gate.yml** の 7 形式。後続の functional-breakdown / feature-decomposition / task-decomposition スキルの入力として使われる。
tab: 設計
builtin: true
---
---
...
1. **Phase 0 Foundation gate を必ずアーキテクチャの一部として定義** — 8 merge gate (lint #1-19 / 3-tier AC validator / RLS coverage / audit MD validator / pytest cov 70% / pyright strict / tsc / mock-impl-diff) が CI に組み込まれる前提で他のレイヤーを設計する。STEP 4 で `v3-gate.yml` テンプレートを必ず出力
2. **RLS strategy を per-entity policy で設計** — STEP 3 で entities 単位の `rls_policies[]` (operation / role / predicate) と `predicates_lib` (self_only / owner_only / account_member / workspace_member) を必須提示。tenant_isolation_pattern も明示
3. **table naming 規約を明示** — PascalCase entity → snake_case_plural table の対応 / `bf_` prefix 禁止 / 予約語衝突時 singular 許可。lint #19 で検証
4. **Vertical Slice 適合性を STEP 2 で必須検証** — 1 機能 = 画面+API+test+RLS が 1 PR で完結する monorepo 構造 / 型同期 / RLS migration 連携を 5 項目で検証
5. **30-50 並列対応を STEP 4 で必須項目化** — git_strategy (worktree / branch 命名 `claude/<task_id>` / squash + auto-merge) / monorepo tool 選定 / 衝突回避プロトコル
6. **ADR 起票プロトコルを出力** — Phase 0 で起票すべき ADR-013 (AUTH) / ADR-014 (命名) / ADR-015 (root画面) 等を `adrs-to-create.json` に列挙し、decision-record スキルへ連携
7. **CI/Lint ツール + monorepo tool を STEP 4.5-D に追加** — pyright / ruff / Playwright / coverage tool / pnpm workspaces / Turborepo / Nx を選定対象に
...
| **v3: Phase 0 Foundation gate 要件** | 8 merge gate (lint #1-19 / 3-tier AC validator / RLS coverage / audit MD validator / pytest cov 70% / pyright strict / tsc / mock-impl-diff) を CI で稼働させて OK か (推奨: yes) |
| **v3: 並列開発セッション数** | Claude Code 並列セッション目標 (30 / 50) — monorepo / worktree / branch 命名規約が必要 |
| **v3: ADR 起票要否** | Phase 0 で必須 ADR (ADR-013 AUTH 戦略 / ADR-014 命名規約 / ADR-015 root画面方針 等) を起票するか |
| **v3: 既存実装の有無 (drift 候補)** | 既存実装あり = functional-breakdown の drift 検知出力が来る → architecture-design は drift 修正容易性 (型同期 / RLS migration 連携) を考慮 |
...
「1 機能 = 画面+API+test+RLS が 1 PR で完結する」をアーキテクチャが支えるかを必ず検証:

| 検証項目 | OK の条件 | NG の場合の修正案 |
|---|---|---|
| ① ディレクトリ構造 | frontend/app/<screen>/ + backend/routers/<resource>.py + backend/tests/test_<task_id>.py + supabase/migrations/<n>_<entity>.sql が同 PR で扱える | monorepo 構造に統一 |
| ② monorepo ビルド | frontend/backend を同 commit でビルド可能 (pnpm workspaces / Turborepo / Nx) | monorepo tool 採用 |
| ③ 型同期 | API スキーマから TS 型を自動生成 (OpenAPI → openapi-typescript / FastAPI → pydantic-to-typescript) | 型生成スクリプトを Phase 0 task 化 |
| ④ RLS migration 連携 | supabase migrations が同 PR に含まれる場合、CI で適用順序保証 | Supabase CLI + migration 番号順 で保証 |
| ⑤ test 一括実行 | 1 PR で backend pytest + frontend vitest + e2e Playwright が並列実行可能 | CI workflow を job split で対応 |

### v3 必須: 30-50 並列開発インフラ検証

| 項目 | 推奨設計 |
|---|---|
| git workflow | trunk-based + worktree |
| branch 命名 | `claude/<task_id>` (例: claude/T-V3-AUTH-01) |
...
| parallel session 上限 | 30 or 50 (Claude Code on web のプラン) |
...
### v3 必須: RLS strategy (per-entity policy)
詳細スキーマ: references/v3-extensions.md "RLS strategy"

```json
{
  "rls_strategy": {
    "default_enable": true,
    "auth_provider": "Supabase Auth | Auth.js | Clerk | 自前 JWT",
...
### v3 必須: table naming 規約
| 項目 | 規約 |
|---|---|
| entity name (functional-breakdown 出力) | PascalCase (例: `User`, `AccountMember`) |
| table name (DB) | snake_case_plural (例: `users`, `account_members`) |
| 禁止 prefix | `bf_`, `tmp_`, `old_` |
| 予約語衝突時 | singular 許可 (例: `user_setting`) |
| 検証 | lint #19 entity-table-naming で自動検出 |
```
...
## v3 必須: CI gate 8 (merge gate)
詳細: references/v3-extensions.md "Phase 0 Foundation gate 要件"

| # | Gate | script | blocking | owner_task |
|---|---|---|:---:|---|
| 1 | mock lint (1-19) | `bash scripts/lint-mock.sh` | ✅ | T-V3-INFRA-02 |
| 2 | 3-tier AC validator | `python3 scripts/validate-ears-ac.py docs/.../tickets.json` | ✅ | T-V3-INFRA-06 |
| 3 | audit MD validator | `python3 scripts/validate-audit-md.py` | ✅ | T-V3-INFRA-06 |
| 4 | RLS coverage | `python3 scripts/verify-rls-coverage.py` | ✅ | T-V3-INFRA-04 |
| 5 | pytest + coverage | `pytest --cov --cov-fail-under=70` | ✅ | T-V3-INFRA-08 |
| 6 | pyright strict | `pyright --strict` | ✅ | T-V3-INFRA-07 |
| 7 | TypeScript strict | `cd frontend && tsc --noEmit && pnpm run lint` | ✅ | T-V3-INFRA-07 |
| 8 | mock-impl diff | `bash scripts/lint-mock-impl-diff.sh ${SCREEN_IDS}` | ✅ (frontend label 時) | T-V3-INFRA-03 |

## v3 必須: GitHub Actions テンプレ (`.github/workflows/v3-gate.yml`)
references/v3-extensions.md の "v3-gate.yml" セクションをそのまま採用。STEP 5 で出力する。

## v3 必須: 30-50 並列開発 git 戦略

```yaml
git_strategy:
  workflow: "trunk-based + worktree"
  branch_naming: "claude/<task_id>"
  ...
  parallel_session_limit: 30  # or 50 (Claude Code プラン依存)
```
...
#### 【出力③】判断ログJSON（データ蓄積・MCP連携向け / v3 では「出力⑦」に番号変更）
...
#### 【出力④】selected-stack.json (STEP 4.5 出力 / 後続スキルへの引き継ぎ)
STEP 4.5 で生成した内容をそのまま出力 (上記 4.5 出力 JSON 参照)。

---

#### 【出力⑤】phase_0_gates.json (v3 新規 / Foundation gate 定義)
...
```json
{
  ...
  "merge_gates": [
    ...
    },
    {
      ...
      "script": "python3 scripts/validate-ears-ac.py docs/task-decomposition/<date>_v3/tickets.json",
      "blocking": true,
    ...
    },
    {
      ...
      "script": "python3 scripts/validate-audit-md.py",
      "blocking": true,
      "owner_task": "T-V3-INFRA-06",
      "detects": ["audit MD 不在", "generic 文言"],
      "depends_on": []
    },
    {
      "id": 4,
      "name": "RLS coverage",
      "script": "python3 scripts/verify-rls-coverage.py",
      "blocking": true,
      "owner_task": "T-V3-INFRA-04",
      "detects": ["entity に RLS policy 不在"],
      "depends_on": []
    },
    {
      "id": 5,
      "name": "pytest + coverage",
      "script": "pytest --cov --cov-fail-under=70",
      "blocking": true,
      "owner_task": "T-V3-INFRA-08",
      "detects": ["unit test 失敗", "カバレッジ < 70%"],
      "depends_on": []
    },
    {
      "id": 6,
      "name": "pyright strict",
      "script": "pyright --strict",
      "blocking": true,
    ...
    {
      "id": 7,
      "name": "TypeScript strict + ESLint",
      "script": "cd frontend && tsc --noEmit && pnpm run lint",
      "blocking": true,
      "owner_task": "T-V3-INFRA-07",
      "detects": ["TS 型エラー", "ESLint 違反"],
      "depends_on": []
    },
    {
      "id": 8,
      "name": "mock-impl diff",
      "script": "bash scripts/lint-mock-impl-diff.sh ${SCREEN_IDS}",
      "blocking_when": "structural AC nonempty",
      "owner_task": "T-V3-INFRA-03",
      "detects": ["mock h1 / KPI / section-h2 と impl 不一致"],
      "depends_on": []
    }
  ],
...
#### 【出力⑥】adrs-to-create.json (v3 新規 / Phase 0 ADR 起票プロトコル)
...
```json
{
  ...
  "phase_0_required_adrs": [
    ...
    {
      "id": "ADR-014",
      "title": "命名規約 (PascalCase entity → snake_case_plural table / forbidden prefixes [bf_, tmp_, old_])",
      "category": "naming",
      "supersedes": [],
      "task_id": "T-V3-INFRA-01",
    ...
    {
      "id": "ADR-015",
      "title": "root画面方針 (root URL の責務 = account dashboard / SaaS root)",
      "category": "ui",
      "supersedes": [],
      "task_id": "T-V3-INFRA-01",
      "rationale": "...",
...
`.github/workflows/v3-gate.yml` として配置:
  ...
  gate:
    ...
    steps:
      ...
      - name: Gate 2 — 3-tier AC validator
        run: python3 scripts/validate-ears-ac.py docs/task-decomposition/<date>_v3/tickets.json
      ...
      - name: Gate 8 — mock-impl diff
        ...
        run: bash scripts/lint-mock-impl-diff.sh ${SCREEN_IDS}
```
...
| Linter (Python) | **ruff** (推奨) / Black + isort / Pylint |
| Linter (TS) | ESLint + Prettier / Biome (高速) |
| Type checker (Python) | **pyright (strict)** (推奨) / mypy |
| Type checker (TS) | tsc strict |
| Test runner (Python) | **pytest + pytest-cov + pytest-asyncio** (推奨) |
...
| **monorepo tool (v3 新規)** | **pnpm workspaces** (Node 系 / 軽量) / **Turborepo** (build cache + 並列 task) / **Nx** (TS+Python deep dep graph) |

**v3 必須**: 各ツールが Phase 0 ゲートに対応:
- pyright → gate #6
- tsc + ESLint → gate #7
- pytest --cov-fail-under=70 → gate #5
- Playwright (UI task) → gate #8 補助
- shellcheck → gate #1 (lint-mock.sh のサブセット)
...
```json
{
  "selections": {
    ...
    "monorepo_tool": {"chosen": "pnpm workspaces", "alternatives": ["Turborepo", "Nx"], "reason": "...", "review": "..."},
    "linter_python": {"chosen": "ruff", "alternatives": ["Black + isort"], "reason": "...", "review": "..."},
    "type_checker_python": {"chosen": "pyright (strict)", "alternatives": ["mypy"], "reason": "...", "review": "..."},
    "test_runner_python": {"chosen": "pytest + pytest-cov", "alternatives": [], "reason": "...", "review": "..."}
  },
