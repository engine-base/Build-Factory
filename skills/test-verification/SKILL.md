...
description: 実装完了コードのテスト戦略設計・品質検証スキル。テスト種類選定 (unit / integration / E2E / contract / access-control matrix)、カバレッジ基準、CI 連携、受け入れ基準を設計する。v3 採用 (2026-05-15〜): tickets.json (3-tier AC) + ears-ac-seed.json を pull し、3-tier AC を test レベルに 1:1 マッピング (structural → mock-impl-diff lint / functional → unit+contract+access-control / regression → CI gate 自動化)。EARS AC から test case 自動生成 (EVENT-DRIVEN → 正常系 / UNWANTED → 異常系 / STATE-DRIVEN → parametrize)。N CI gate を Foundation → Backend → UI → Polish の段階で構成。Access control matrix test (role × operation) を verifier で網羅。Schemathesis + Pact を contract test に採用。連続 N 失敗で human エスカ、全 pass で auto-merge。「テストを書きたい」「テスト戦略を決めたい」「品質基準を設けたい」「CI を整備したい」「リグレッションを防ぎたい」「3-tier AC を test に落としたい」「EARS AC から test を生成したい」「access-control を role × operation matrix でテストしたい」「N CI gate を段階構成したい」「mock-impl drift を防ぎたい」「contract test を使いたい」「gate-config.yml を作りたい」「ears-test-mapping.json を作りたい」場面で必ず使う。4STEP の対話型プロセス。出力はテスト計画書 + テスト設計 JSON + gate-config.yml + ears-test-mapping.json の 4 形式。
tab: 品質・運用
builtin: true
---
---
