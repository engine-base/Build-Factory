import type { IFrameView } from '@/components/design-canvas/canvas/frame/view';
// TODO: Build-Factory integration - removed Onlook dependency (@/trpc/client, @onlook/db)
import { type Frame } from '@/lib/onlook/models';

// TODO: Build-Factory integration - replace with REST/GraphQL persistence
const api = {
    frame: {
        delete: { mutate: async (_args: unknown): Promise<boolean> => true },
        create: { mutate: async (_args: unknown, _opts?: unknown): Promise<boolean> => true },
        update: { mutate: async (_args: unknown): Promise<boolean> => true },
    },
} as const;

// TODO: Build-Factory integration - identity passthroughs (was @onlook/db serializers)
const toDbFrame = (frame: Frame): Frame => frame;
const toDbPartialFrame = (frame: Partial<Frame>): Partial<Frame> => frame;
import { calculateNonOverlappingPosition } from '@/lib/onlook/utility';
