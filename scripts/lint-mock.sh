...
# ----------------------------------------------------------------
check_archive() {
  echo "[3/15] ARCHIVE 対象 (onlook/penpot) 残留検出..."
  local found=0
...
# ----------------------------------------------------------------
# 15. T-AI-MEM-02 AC-4 UNWANTED: server-side compaction 自前実装の禁止語検知
...
check_no_self_compaction() {
  echo "[15/15] server-side compaction 自前実装検知 (T-AI-MEM-02 AC-4)..."
...
case "$MODE" in
  ...
  all|"")
    ...
    ;;
  *)
    echo "Usage: $0 [--emoji|--agpl|--archive|--tickets|--secrets|--no-langgraph|--no-litellm-in-runner|--domains|--no-self-provider-routing|--no-self-tool-trim|--template-skeleton|--no-self-constitution|--no-self-fallback-circuit|--no-self-handoff|--no-self-compaction|all]"
    exit 2
    ;;
esac
