...
[アーキテクト] architecture-design + tech-stack で設計
    ↓
[アーキテクト] api-design で API 仕様確定
    ↓
─────────── Phase A: デザイン確定 (feature-decomposition の前) ──
[デザイン担当 AI] brand-voice → design-md → frontend-design → ui-mockup
    ・brand-voice: ブランドの声・トーン (任意・自社プロダクトでは省略可)
    ・design-md: Google Labs 形式のデザインシステム (色 / タイポ / 間隔 / コンポーネント)
    ・frontend-design: スタイル方針 (Editorial / Brutalism / Minimal / Glassmorphism 等)
    ・ui-mockup: デザイントークン確定後の UI モック方針
    ↓
─────────── Phase B: 完成モック生成 (Onlook + Open Design 資産) ──
[Build-Factory] が Onlook プロジェクトを自動作成
    ・Open Design の 71 DESIGN.md から最も近いブランド指針を流用
    ・Open Design の 31 SKILL.md から該当パターンを Onlook prompt に変換
    ・確定したデザインスキル出力を Onlook に渡す
    ↓
[ユーザー] Onlook で全画面まとめて土台モック生成 (1 個生成・5 候補なし)
    ・Figma 風キャンバスで全画面を一覧
    ・要素クリックで AI 編集
    ・要素にコメント (クライアント招待可)
    ・確定後 Next.js + Tailwind コードとして Build-Factory に取り込み
    ↓
─────────── Phase C: タスク分解 (完成モック後) ─────────
[シニアエンジニア] feature-decomposition + task-decomposition でタスク分解
    ・完成モック参照しながら分解
    ・各タスクに「モック内の該当部分（コンポーネント）」を紐付け
    ↓
...
| M-10 | コメント機能（Figma/Notion 風 + AI 一括反映） | 要件・モックへ注釈 (モックは Onlook のコメント機能を活用) |
| M-10b | **Onlook 連携 (デザイン編集レイヤー)** | self-host・OAuth SSO・engine-base.com/design |
| M-10c | **デザインスキル統合** (Phase A) | brand-voice / design-md / frontend-design / ui-mockup |
| M-10d | **Open Design 資産取り込み** | 71 DESIGN.md + 31 SKILL.md を `data/design-systems/` `data/skills/` に |
...
## 9.4. デザイン編集レイヤー: Onlook + Open Design（重要）

### 経緯

「チャットだけだとデザインが思った通りに作れない」問題を解決するため、外部 OSS を統合。
複数 OSS を比較した結果、**Onlook をメイン編集ツール + Open Design を資産（テンプレート）として流用** する構成が最適と判断。

### 採用 OSS 一覧

| OSS | ライセンス | 役割 | URL |
|---|---|---|---|
| **Onlook** | Apache 2.0 | デザイン編集 (Figma 風キャンバス + AI 編集 + コメント) | https://github.com/onlook-dev/onlook |
| **Open Design (nexu-io)** | Apache 2.0 | デザインシステム資産（71 DESIGN.md）+ スキル資産（31 SKILL.md） | https://github.com/nexu-io/open-design |

不採用（評価済）:
- Open CoDesign (OpenCoworkAI): Electron デスクトップ専用
- OpenPencil: Electron + MCP・GUI 機能は Onlook と重複
- Penpot: Web で良いが AI 生成が弱い・MPL-2.0 で SaaS 制約あり
- 自作 GUI レイヤー: 工数とメンテコストが見合わない

### Onlook の機能（採用理由）

- ✅ **Figma 風無限キャンバス** で全画面を一覧表示
- ✅ **要素クリックで AI 編集**（コードと同期）
- ✅ **要素にコメント機能**
- ✅ **共同編集**（クライアント招待可）
- ✅ **MCP 対応**（Build-Factory との連携可能）
- ✅ **Apache 2.0**（SaaS 商用 OK）
- ✅ **Web ベース**（Desktop 廃止して Web に統合済）
- ✅ Next.js + Tailwind 出力 → Claude Code に渡せるコード形式

### Open Design の活用範囲

**「資産取り込み」のみ・本体採用なし**:
- **71 DESIGN.md**（Linear / Stripe / Vercel / Airbnb / Notion / Apple 等のデザインシステム）
  → Build-Factory の `data/design-systems/` に取り込み・初期 prompt として流用
- **31 SKILL.md**（saas-landing / dashboard / mobile-app 等のパターン）
  → Build-Factory の skill ライブラリに追加
- **5 候補ピッカー方式は採用しない**（土台 1 個生成 + Onlook で反復編集の方が効率的）

### 統合戦略

#### Phase 1（MVP）: Onlook を **OSS 素のまま self-host**

```
[ docker-compose ]
  ├─ Build-Factory (8001 / 3001)
  └─ Onlook (素のまま起動・engine-base.com/design 配信)
       ↓ リバースプロキシで「同じドメイン下の別パス」
       OAuth で Build-Factory と SSO 連携
       1 アプリ感を出すが実態は別プロセス
```

**判断理由**:
- Build-Factory 本丸（リーダー AI 壁打ち / 仕様書工場 / Claude Code 連携）に集中したい
- デザイン編集は全機能の 10〜15% → fork に 4-5 日 + 月 1-2 日の維持コストは投資配分悪い
- Onlook は急速進化中 → 上流追従の利益が大きい
- 「使い込んでから fork」が王道（Cursor / Linear も同じパターン）

#### Phase 2 以降のフォーク判断指標

以下のいずれかが起きたら fork を検討:
- Onlook の独自機能の **50% 以上を使っていない**
- Build-Factory の AI 社員 と Onlook の AI チャットが **画面分離して使いづらい**
- Onlook の上流変更が **Build-Factory 体験を壊す**
- 監視担当・クライアントから **「2 タブで作業しづらい」と苦情**
...
```
[Build-Factory Workspace] Design タブ
  ↓
Step 1〜6: brand-voice / design-md / frontend-design 等で方針確定
  ↓
Step 7: 完成モック生成
  ・選択: Open Design のデザインシステム (71 から)
  ・選択: Open Design のスキル (31 から)
  ・[ Onlook で開く ] ボタン
  ↓
別タブ（or リバースプロキシ経由で /design）で Onlook が開く
  ・自動でプロジェクト作成（Build-Factory から context 受け渡し）
  ・全画面まとめて土台生成（5 候補ではなく 1 個）
  ↓
Onlook で反復編集
  ・要素クリック → AI 編集 / コメント / GUI 直編集
  ・クライアントもコメントで参加可（共同編集）
  ↓
確定 → Build-Factory に Next.js + Tailwind コードが取り込まれる
  ↓
Phase C 機能分解 → Phase D 実装（Claude Code）
```
...
**Phase A デザイン確定用** ★（既存・Build-Factory で組込）:
| スキル | 用途 |
|---|---|
| `brand-voice` | ブランドボイス・トーン |
| `design-md` | Google Labs 形式デザインシステム確定 |
| `frontend-design` | UI スタイル方針（Editorial / Brutalism / Minimal 等）|
| `ui-mockup` | デザイントークン → UI モック方針 |
...
| D-17 | デザイン編集レイヤーに Onlook 採用 (Apache 2.0) | Figma 風キャンバス + GUI 編集 + AI 編集 + コメントが既に実装済 | Open Design / Open CoDesign / OpenPencil / Penpot / 自作 | 機能不足 / Electron 専用 / AI 弱い / 工数過大 |
| D-18 | Open Design は 71 DESIGN.md + 31 SKILL.md の資産流用のみ | デザインシステム蓄積 100+ を即時取り込み | 本体採用 / 採用しない | GUI 編集弱い / 資産が惜しい |
| D-19 | 5 候補ピッカー方式は不採用・土台 1 個生成 + Onlook で反復編集 | 設計確定後ならブレ少ない・Onlook 編集の方が効率的 | 5 候補方式 | コスト 5 倍・選択疲労 |
| D-20 | モック生成は **タスク分解の前に全画面完成** | 全体一貫性確保・タスク分解時に該当部分抜粋可 | タスクごとに個別モック | 一貫性低下・ブレ |
| D-21 | Phase A にデザインスキル群を明示組込（brand-voice / design-md / frontend-design / ui-mockup） | 設計トークン確定 → Onlook に渡す流れ | Onlook 内部だけで完結 | 既存スキル資産が活きない |
| D-22 | Onlook は **OSS 素のまま self-host**（Phase 1 はフォークしない） | Build-Factory MVP 集中・上流追従の利益・「使ってから判断」 | 最初からフォーク | 工数過大・メンテ義務 |
| D-23 | Onlook はリバースプロキシで `engine-base.com/design` に配信（1 アプリ感） | iframe 埋め込みは嫌・別タブも避けたい中間案 | 別タブ / iframe 埋込 | 体験悪化 / 重い |
